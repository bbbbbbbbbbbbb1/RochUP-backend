...
func joinMeeting(db *gorm.DB, userId string, meetingId int) (bool, string, time.Time, []string, []string, []int) {
	...
	if user_info.Error == nil && meeting_info.Error == nil {
		participant_info := db.First(&participant, "meeting_id = ? AND user_id = ?", meetingId, userId).Delete(&participant, "meeting_id = ? AND user_id = ?", meetingId, userId).Update("is_joining", true)
